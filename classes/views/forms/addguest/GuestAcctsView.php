...
class GuestAcctsView extends BaseView
{
    ...
    public function getFieldLabels(): array
    {
        ...
        return [
            ...
            'acctname'   => 'Account name/label',
            'uid'        => 'Account UID',
            'gid'        => 'Account GID',
            'shell'      => 'Account shell',
            'passwd'     => 'Account password',
            'accttype'   => 'Type of account',
            'expiredays' => 'Time to expire the guest account',
        ];
    }
    ...
    protected function getPageContents(
        ...
        switch ($formData['command']) {
            case 'clearguest':
                ...
                return $this->generateClearguestFormPage($this->debug->isDebugMode(), $formData);
                break;
            case 'createguest':
                // handle createguest command;
                return $this->generateCreateguestFormPage($this->debug->isDebugMode(), $formData);
                break;
            case 'extendguest':
                // handle extendguest command;
                return $this->generateExtendguestFormPage($this->debug->isDebugMode(), $formData);
                break;
            case 'removeguest':
                // handle removeguest command;
                return $this->generateRemoveguestFormPage($this->debug->isDebugMode(), $formData);
                break;
            case 'addguest':
            default:
                // handle addguest command;
                return $this->generateAddguestFormPage($this->debug->isDebugMode(), $formData);
                break;
        }
    }
}
